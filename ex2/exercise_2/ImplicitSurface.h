...
public:
	...
	{
		// implement the implicit sphere formula using the member variables m_center and m_radius
...
public:
	...
	double Eval(const Eigen::Vector3d& _x)
	{
		// implement the implicit torus formula using the  variables m_center, m_radius (radius of the ring) and the radius m_a (small radius)
		double x2 = pow(_x.x() - m_center.x(), 2);
		double y2 = pow(_x.y() - m_center.y(), 2);
		double z2 = pow(_x.z() - m_center.z(), 2);
		double R2 = pow(m_radius, 2);
		double a2 = pow(m_a, 2);
		return pow(x2 + y2 + z2 + R2 - a2, 2) - 4 * R2 * (x2 + y2);
	}
...
public:
	...
	double Eval(const Eigen::Vector3d& _x)
	{
		...
		// implement the evaluation using Hoppe's method (see lecture slides)
		Vector3f xp = x - p;
		return xp.dot(n);
	}
...
public:
	...
	double Eval(const Eigen::Vector3d& _x)
	{
		...
		result += Vector3d(m_coefficents[m_numCenters], m_coefficents[m_numCenters + 1], m_coefficents[m_numCenters + 2]).dot(_x);
...
#define phi(i,j) EvalBasis((m_funcSamp.m_pos[i]-m_funcSamp.m_pos[j]).norm())
	...
	void BuildSystem()
	{
		...
		int height = 2 * m_numCenters;
		int width = m_numCenters + 4;
