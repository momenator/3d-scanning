...
public:
	...
	bool operator()(const T* const pose, T* residuals) const {
		// Implement the point-to-point cost function.
		...
		// outputVector is the translated vector!
		T inputVector[3], outputVector[3];
		inputVector[0] = T(m_sourcePoint.x());
		inputVector[1] = T(m_sourcePoint.y());
		inputVector[2] = T(m_sourcePoint.z());

		PoseIncrement<T> poseIncrement(poseArray);
		poseIncrement.apply(inputVector, outputVector);

		residuals[0] = (T(m_weight) * outputVector[0]) - T(m_targetPoint(0));
		residuals[1] = (T(m_weight) * outputVector[1]) - T(m_targetPoint(1));
		residuals[2] = (T(m_weight) * outputVector[2]) - T(m_targetPoint(2));
...
public:
	...
	template <typename T>
	bool operator()(const T* const pose, T* residuals) const {
		// Implement the point-to-plane cost function.
		...
		auto xTerm = T(m_targetNormal(0)) * ((T(m_weight) * outputVector[0]) - T(m_targetPoint(0)));
		auto yTerm = T(m_targetNormal(1)) * ((T(m_weight) * outputVector[1]) - T(m_targetPoint(1)));
		auto zTerm = T(m_targetNormal(2)) * ((T(m_weight) * outputVector[2]) - T(m_targetPoint(2)));

		residuals[0] = xTerm + yTerm + zTerm;
	...
	void pruneCorrespondences(const std::vector<Vector3f>& sourceNormals, const std::vector<Vector3f>& targetNormals, std::vector<Match>& matches) {
		...
		for (unsigned i = 0; i < nPoints; i++) {
			...
			if (match.idx >= 0) {
				...
				auto cosAngle = acos(nominator/sqrt(denominator));				
...
private:
	...
	Matrix4f estimatePosePointToPlane(const std::vector<Vector3f>& sourcePoints, const std::vector<Vector3f>& targetPoints, const std::vector<Vector3f>& targetNormals) {
		...
		float pointToPlaneWeight = 25.0f;
		...
		for (unsigned i = 0; i < nPoints; i++) {
			...
			A(i * 4, 0) = pointToPlaneWeight * (n.z() * s.y() - n.y() * s.z());
			A(i * 4, 1) = pointToPlaneWeight * (n.x() * s.z() - n.z() * s.x());
			A(i * 4, 2) = pointToPlaneWeight * (n.y() * s.x() - n.x() * s.y());
			A(i * 4, 3) = pointToPlaneWeight * n.x();
			A(i * 4, 4) = pointToPlaneWeight * n.y();
			A(i * 4, 5) = pointToPlaneWeight * n.z();
			b(i * 4) = pointToPlaneWeight * (n.x() * d.x() + n.y() * d.y() + n.z() * d.z() - n.x() * s.x() - n.y() * s.y() - n.z() * s.z());
